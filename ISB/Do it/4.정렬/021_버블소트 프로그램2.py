...
머지소트로 구현하려고 했으나, 인덱스가 이동한 거리를 구하는 동작에서 시간초과 없이 구하는 방법이 떠오르지 않았다.

해답을 보니, 인덱스이동거리 구하는 식은 생각보다 아주아주 간단했다.
각 반복문에서 인덱스 이동거리 = 왼쪽 배열 길이 - 왼쪽 배열의 현재 인덱스
...
def divide_conquer(arr):
    if len(arr) == 1:
        ...
        A = arr[:len(arr)//2]
        B = arr[len(arr)//2:]
        return conquer(divide_conquer(A), divide_conquer(B))
